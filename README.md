...
Authentication flow
Create a merchant user directly in the database or through a one-time seed step.
Log in with POST /api/auth/login to get an access token.
Use the token as:
Authorization: Bearer <access_token>
Use the role-based endpoints to manage users and store operations.
Public endpoints
...
POST /api/auth/login with the merchant credentials
POST /api/auth/invitations to invite an admin
The admin completes registration via POST /api/auth/register-admin
The admin can then create clerks using the clerk management endpoint
...
After the initial deployment, set CORS_ALLOWED_ORIGINS to your frontend domain, such as:
https://myduka.vercel.app
Deploy the app and confirm the health endpoint:
